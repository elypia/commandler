...
public class HelpModule implements Handler {
    ...
    @Inject
    public HelpModule(Context context, LanguageAdapter language, TestRunner testRunner) {
    ...
    public String listModules(CommandlerEvent<?> event, HelpModulesParams params) {
        var groups = context.getGroups(false);

        Optional<String> optGroupName = groups.keySet().stream().filter(
            name -> name.equalsIgnoreCase(params.getQuery())
        ).findAny();
    ...
    public String listCommands(CommandlerEvent event, MetaModule module) {
        ...
        Iterator<MetaCommand> metaCommandIt = getModule().getPublicCommands().iterator();

        while (metaCommandIt.hasNext()) {
            MetaCommand command = metaCommandIt.next();
            builder.append(language.get(event.source(), command.getName()));

            builder
                    .append(" (")
                    .append(String.join(", ", command.getAliases()))
                    .append(")\n")
                    .append(language.get(event.source(), command.getHelp()));

            List<ParamData> params = command.getParams();

            params.forEach((param) -> {
                builder.append("\n" + language.get(event.source(), param.getName()) + ": ");
                builder.append(language.get(event.source(), param.getHelp()));
            });
